...
const EditorMonaco = ({ setIncomingJSON, incomingCode, readOnly = false, height = "", width = "", isDynamic}) => {
  ...
  useEffect(() => {
    // Set the initial editor content with incomingCode only if code is empty
  ...
  function handleEditorChange(value) {
    setCode(value || ''); // Update code state without modifying incomingCode
  }
  ...
  async function handleSubmit() {
    try {
      ...
      setIncomingJSON(response.data); // Update visualization data without modifying editor content
    } catch (error) {
